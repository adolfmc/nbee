...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response){
        return HttpTools.executeLogin(request);
    }

    @Override
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        log.info("PermissionAuthorizationFilter执行");
        boolean isContainsStaticc = request.getRealPath("./").contains("static");
        if(isContainsStaticc==true){
            return true;
        }
        org.apache.shiro.web.servlet.ShiroHttpServletRequest srequest = (org.apache.shiro.web.servlet.ShiroHttpServletRequest)request;
        boolean isContainsprovider = srequest.getRequestURI().contains("queryTeasByChennel");
        if(isContainsprovider==true){
            return true;
        }
